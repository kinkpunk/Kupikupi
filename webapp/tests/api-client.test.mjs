...
test("updateShoppingRequest puts edited text and constraints", async () => {
  ...
  const client = createApiClient({
    ...
    fetchImpl: async (url, init) => {
      calls.push({ url, init });
      return jsonResponse(200, { id: "request-1", raw_text: "Edited request" });
    },
  });
...
test("restoreWatchlist posts restore endpoint", async () => {
  const calls = [];
  const client = createApiClient({
    baseUrl: "https://api.example.test/v1",
    accessToken: "token",
  ...
  await client.restoreWatchlist("watchlist-1");

  assert.equal(calls[0].url, "https://api.example.test/v1/watchlists/watchlist-1/restore");
  assert.equal(calls[0].init.method, "POST");
});
