...
- Telegram Bot uses a static backend access token mode; it does not yet authenticate each Telegram
  user against the backend user/session flow.
...
2. Replace Telegram Bot static backend token usage with Telegram user authentication/session flow.
3. Add staging-safe WebApp auth path without `NEXT_PUBLIC_DEMO_ACCESS_TOKEN`.
4. Add one real store source config path, preferably a stable affiliate/XML/JSON feed before
   scrapers.
5. Add live FX-rate updater job and migration-safe seed/fallback behavior.
6. Add basic observability: JSON logs, request IDs, Sentry-compatible error hooks, and container
   health dashboard notes.
7. Add backup/restore instructions for PostgreSQL.
8. Run end-to-end Docker Compose smoke on a machine with Docker available.

## Go/No-Go Summary

- Local developer demo: go.
- Internal technical demo with demo data: go.
- Closed user test with real Telegram users: no-go until Telegram auth/session and staging HTTPS are
  in place.
